...
2.  **Submit to a Journal (Publish its Location):** You upload your data to a public storage system (like IPFS) and then register its _location_ on the LazAI blockchain. You are not putting the data itself on the chain, just a pointer to it.
...
4.  **Get Published (Receive a Proof):** The node examines your data and, if it meets the criteria, submits a "proof" to the blockchain. This is like your paper being officially accepted and published. Your data is now trusted.
...
Then, we use our LazAI Client to register this URL on the LazAI blockchain. This creates a permanent, public record pointing to our data.
...
# Register the URL on the LazAI blockchain
...
The process is a coordinated effort between you (the Contributor), the LazAI Blockchain, and a Proof Node.
...
sequenceDiagram
    ...
    participant BC as "LazAI Blockchain"
